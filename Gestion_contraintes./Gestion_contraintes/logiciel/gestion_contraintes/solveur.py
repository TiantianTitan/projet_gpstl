...
from contraintesPersonnalisees import ajouter_contraintes
import re
    ...
    for etu in liste_etudiants:
        ...
        for nom_ue in etu.listes_ue():
            ...
            for cours in ue.creneaux_cours:
                nom_cours = etu.nom_variable + "{}{}{}".format(Parametres.separateur_var + nom_ue, Parametres.separateur_var + "cours", Parametres.separateur_var + str(cpt))
                var_cours = modele.addVar(vtype=GRB.BINARY, name=nom_cours)
                vars_cours_par_ue[etu][ue].append(var_cours)

                creneau_cours = ue.creneaux_cours[0]
                if creneau_cours > 0 :
                ...
                if len(ue.creneaux_cours) > 1:
                    creneau_cours = ue.creneaux_cours[1]
                    if creneau_cours > 0 :
                    	if creneau_cours not in vars_creneaux_incompatibles[etu]:
                    		vars_creneaux_incompatibles[etu][creneau_cours] = []
                    	vars_creneaux_incompatibles[etu][creneau_cours].append(var_cours)
            ...
            for k in range(ue.nb_groupes):
                if not k in vars_x_par_ue[ue]:
                    vars_x_par_ue[ue][k] = []
                nom = etu.nom_variable + "{}{}".format(Parametres.separateur_var + nom_ue, Parametres.separateur_var + str(k))
...
def decompose_var_x(nom_var):
    """Renvoie le parcours, l'ue et groupe a partir d'un nom de variable x"""
    m = re.search("x{}.*{}(.*){}(.*){}(.*)".format(Parametres.separateur_var, Parametres.separateur_var, Parametres.separateur_var, Parametres.separateur_var), nom_var)
    return (m.group(1), m.group(2), str(int(m.group(3))+1))

def decompose_var_y(nom_var):
    """Renvoie l'ue a partir d'un nom de variable y"""
    m = re.search("y{}.*{}(.*)".format(Parametres.separateur_var, Parametres.separateur_var), nom_var)
    return m.group(1)
